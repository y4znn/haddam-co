                        ...
                        >
                            <Command className="w-full bg-transparent" shouldFilter={false}>
                                <div className="flex items-center border-b border-white/10 px-4 relative">
                                    <Search className="w-5 h-5 text-brand-core mr-2" />
                                    ...
                                    <Command.Group heading="Neural Predictions" className="text-xs font-bold text-muted-foreground uppercase tracking-widest px-2 py-2">
                                        {products.filter(p => p.name.toLowerCase().includes(query.toLowerCase())).slice(0, 5).map((product) => (
                                            ...
                                            >
                                                ...
                                                <div className="absolute left-0 top-0 bottom-0 w-1 bg-brand-core opacity-0 group-aria-selected:opacity-100 transition-opacity shadow-[0_0_15px_var(--brand-core)]" />

                                                {/* Mini-Bento Image */}
                                                <div className="relative w-12 h-12 rounded-lg bg-white/5 overflow-hidden shrink-0 border border-white/10 group-aria-selected:border-brand-core/50 transition-colors">
                                                    <Image
                                                        src={product.image}
                                                        alt={product.name}
                                                        fill
                                                ...
                                                <div className="flex-1 min-w-0 flex flex-col justify-center">
                                                    <div className="flex items-center gap-2 mb-1">
                                                        <span className="font-sans font-bold text-lg tracking-tight text-white group-aria-selected:text-brand-core transition-colors truncate flex-1">
                                                            {product.name}
                                                        </span>
                                                    ...
                                                    <div className="flex items-center justify-between">
                                                        <span className="font-mono text-xs text-brand-core tracking-widest uppercase group-aria-selected:text-white transition-colors">
                                                ...
                                                <ChevronRight className="w-5 h-5 text-brand-core opacity-0 -translate-x-4 group-aria-selected:opacity-100 group-aria-selected:translate-x-0 transition-all duration-300" />
                                            </Command.Item>
                                        ))}
                                    </Command.Group>
                                ...
                                <div className="border-t border-white/10 px-4 py-3 flex items-center justify-between text-[10px] text-muted-foreground font-mono uppercase tracking-[0.2em] bg-black/40">
                                    ...
                                    <span className="flex items-center gap-2">
                                        <span className="w-1.5 h-1.5 rounded-full bg-brand-core animate-pulse shadow-[0_0_10px_var(--brand-core)]" />
                                        <span className="text-brand-core">SYSTEM ONLINE</span>
                                    </span>
                                </div>
                            </Command>
                        </motion.div>
